...
public class MoizaController {
	...
	public String saveUser(@ModelAttribute("user") UserEntity user,
		...
		// 어떤 이유로 든 구성된 암호를 인코딩하지 않으려면 NoOpPasswordEncoder를 사용하여야 하는데 이를 위해 패스워드에
		// {noop}식별자를 붙여준다
	...
	@PreAuthorize("isAuthenticated()") // @PreAuthorize 요청이 들어와 함수를 실행하기 전에 권한을 검사하는 어노테이션 ,isAuthenticated()현재 사용자가
										// 익명이 아니라면 (로그인 상태라면) true
	...
	@PreAuthorize("isAuthenticated()")
	public String ViewGroupMembers(@RequestParam("mgroupIndex") int mgroupIndex, Authentication authentication,
			Model theModel) {
		String userId = authentication.getName();
		int userIndex = moizaService.UseridChangeUserindex(userId);
	...
	public String beforeGroupCreation(Model theModel,
			@RequestParam(value = "img_index", defaultValue = "0") int img_index) {
...
//         }
	...
	public String groupCreation(Authentication authentication, @ModelAttribute("mgroup") MgroupEntity mgroup) {
		...
		if (mgroup.getMgroup_title() == null || mgroup.getMgroup_title().length() == 0 || mgroup.getMgroup_img() == 0
				|| mgroup.getMgroup_maincategory().equals("0") || mgroup.getMgroup_middlecategory().equals("0")) {
	...
	public String joingroup(@RequestParam("mgroupIndex") int mgroupIndex, Authentication authentication,
		...
		int userIndex = moizaService.UseridChangeUserindex(userId);
		// 유저그룹 테이블의 유저그룹인덱스를 가져와야
		// List<UsergroupEntity> theUserGroup = moizaService.getUserGroup(userIndex,
		// mgroupIndex);
	...
	public String modification(@RequestParam("userId") String userId, Model themodel) {
		...
		theUsers.get(0).setPassword(theUsers.get(0).getPassword().substring(6));
	...
	@PreAuthorize("isAuthenticated()")
	public String HandOverSeats(@RequestParam("usergroup_index") int usergroup_index,
			@RequestParam("mgroupIndex") int mgroupIndex, Authentication authentication, Model theModel) {
		// 모임장 위임 하기
		UsergroupEntity UsergroupInfo = moizaService.getUsergroupInfo(usergroup_index);
	...
	public String deletePost(@RequestParam("mgroupIndex") int groupIndex, @RequestParam("post_index") int post_index,
	...
	@PreAuthorize("isAuthenticated()")
	public String writing_post(@RequestParam("mgroupIndex") int mgroupIndex, @RequestParam("post_index") int post_index,
			@RequestParam("post_usergroup_index") int post_usergroup_index,
			@RequestParam("post_maintext") String post_maintext, HttpServletRequest request,
	...
	@PreAuthorize("isAuthenticated()")
	public String writing_post(@ModelAttribute("editedPost") PostEntity editedPost, HttpServletRequest request,
			Authentication authentication, Model theModel) {
